...
def split_paragraphs(text):
    """
    Attempt to split text into paragraphs.

    - First, try to split on double newlines (blank lines).
    - If too few paragraphs are found, fall back to sentence-end-based splitting.
    """

    # Step 1: Try splitting on double newlines
    paras = re.split(r'\n\s*\n', text.strip())
    paras = [re.sub(r'\s*\n\s*', ' ', p.strip()) for p in paras if p.strip()]

    # If most of the text is in one big block (as is common with PDFs), fall back
    if len(paras) < 5:
        # Split based on sentence endings followed by a capital letter
        paras = re.split(r'(?<=[\.\?\!])\s+(?=[A-Z])', text.strip())
        paras = [re.sub(r'\s*\n\s*', ' ', p.strip()) for p in paras if p.strip()]
